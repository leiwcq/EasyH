...
{
    public class NetCorePclExport : PclExport
    {
        public static NetCorePclExport Provider = new NetCorePclExport();
        ...
        {
            PlatformName = Platforms.NetCore;
#if NETSTANDARD2_0
            this.DirSep = Path.DirectorySeparatorChar;
#else 
            DirSep = RuntimeInformation.IsOSPlatform(OSPlatform.Windows) ? '\\' : '/';
#endif
            var req = WebRequest.Create("http://easyh.net");
            try
            {
                req.Headers[HttpRequestHeader.UserAgent] = "EasyH";
