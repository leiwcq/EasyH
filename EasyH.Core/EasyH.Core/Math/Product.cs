...
namespace EasyH.Core.Math
{
    ...
    ///
    /// Use the <see cref="P:Common.Product.Item(System.Int32)">indexer</see>
    ...
    ///
    /// Use the <see cref="P:Permute">Permute</see>
    ...
    ///     <see cref="P:Common.Product.Item(System.Int32)">indexer</see>.
    ///   </li>
    ///   <li>
    ...
    public class Product :
        ...
        IEnumerable<int>
    {
        private readonly int[] _sizes; // Size of each column.
        private readonly long[] _factors; // Running multiple of sizes.
        private readonly long _rowCount; // Row count of the table of products.
        ...
        public Product(Product source)
        {
            ...
            _sizes = new int[source._sizes.Length];
            _factors = new long[source._factors.Length];

            source._sizes.CopyTo(_sizes, 0);
            source._factors.CopyTo(_factors, 0);
        ...
        public Product(int[] sizes)
        {
            ...
            sizes.CopyTo(_sizes, 0);

            _factors = new long[_sizes.Length];
            _rowCount = sizes.Length == 0 ? 0 : 1;

            for (var ei = _sizes.Length - 1; ei >= 0; --ei)
            {
                if (_sizes[ei] < 0)
                    throw new ArgumentOutOfRangeException("sizes", "Value is less than zero.");

                _factors[ei] = _rowCount;
                _rowCount = checked (_rowCount*_sizes[ei]);
            }
        }
            ...
            : this(sizes)
        {
            ...
            for (var si = 0; si < source.Length; ++si)
            {
                ...
                _rank = _rank*sizes[si] + source[si];
            }
        }
        ...
        public int this[int index]
        {
            get
            {
                var rankToElement = _rank;
                if (index > 0)
                    rankToElement %= _factors[index - 1];

                return (int) (rankToElement/_factors[index]);
            }
        }
        ...
        public IEnumerable<Product> GetRows()
        {
            ...
            {
                var startRank = Rank;
                for (var current = (Product) MemberwiseClone();;)
                {
        ...
        public static List<T> Permute<T>(Product arrangement, IList<IList<T>> source)
        {
            ...
            for (var ai = 0; ai < arrangement.Width; ++ai)
