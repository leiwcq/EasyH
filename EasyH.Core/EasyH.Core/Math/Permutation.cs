...
namespace EasyH.Core.Math
{
    ...
    /// 
    /// Use the <see cref="P:Common.Permutation.Item(System.Int32)">indexer</see>
    ...
    /// 
    /// Use the <see cref="P:Permute">Permute</see> method to
    ...
    ///     <see cref="P:Common.Permutation.Item(System.Int32)">indexer</see>.
    ///   </li>
    ///   <li>
    ///     Use the <see cref="P:Permute">Permute</see> method and output the rearranged values.
    ///   </li>
    ///   <li>
    ...
    public class Permutation :
        ...
        IEnumerable<int>
    {
        private int[] _data; // The arrangement for the current rank. Length is 'k'.
        private int _choices; // Number of possible values 'n'.
        private long _rowCount; // Row count of the table of (k-)permutations.
        private long _rank; // Row index.
        ...
        public Permutation(int choices)
        {
            ...
            _choices = choices;
            _rowCount = choices == 0 ? 0 : Combinatoric.factorial[choices];
            _rank = 0;
        }
        ...
        public Permutation(int choices, int picks, long rank)
        {
            ...
            _data = new int[picks];
            _choices = choices;
            _rowCount = CalcCount(choices, picks);
            Rank = rank;
        }
        ...
        private static void Construct(Permutation pn, int[] source)
        {
            if (source.Length > MaxChoices)
                throw new ArgumentOutOfRangeException("source", "Too many values.");

            if (pn._choices < 0 || pn._choices > MaxChoices)
                throw new ArgumentOutOfRangeException("choices");

            pn._data = new int[source.Length];
            source.CopyTo(pn._data, 0);

            var isUsed = new bool[pn.Choices];
            for (var ei = 0; ei < pn.Picks; ++ei)
            {
                if (pn._data[ei] < 0 || pn._data[ei] >= pn.Choices)
                    throw new ArgumentOutOfRangeException("source", "Value is out of range.");

                if (isUsed[pn._data[ei]])
                    throw new ArgumentException("Value is repeated.", "source");
                isUsed[pn._data[ei]] = true;
            }

            pn._rowCount = CalcCount(pn.Choices, pn.Picks);
            pn._rank = CalcRank(pn._data, pn.Choices);
        }
        ...
        public long PlainRank
        {
            ...
            set
            {
                ...
                CalcPlainUnrank(_data, value);
                _rank = CalcRank(_data, Choices);
            }
        }
        ...
        public long Rank
        {
            ...
            set
            {
                ...
                for (var fi = Choices - 1; fi >= 0; --fi)
                    factoradic[fi] = (int) System.Math.DivRem(value, Combinatoric.Factorial(fi), out value);

                // Build the permutation from the diminishing factoradic.
                for (var fi = Choices - 1; fi >= Choices - Picks; --fi)
                    for (var newAtom = 0;; ++newAtom)
                        if (!isUsed[newAtom])
                            if (factoradic[fi] > 0)
                                --factoradic[fi];
                            else
                            {
                                _data[Choices - fi - 1] = newAtom;
                                isUsed[newAtom] = true;
                                break;
                            }
            }
        ...
        public int Backtrack(int nodeIndex)
        {
            ...
            if (nodeIndex < 0 || nodeIndex >= _data.Length)
                throw new ArgumentOutOfRangeException("nodeIndex", "Value is out of range.");

            Array.Sort(_data, nodeIndex + 1, _data.Length - nodeIndex - 1);
            for (var tailIndex = nodeIndex + 1; tailIndex < _data.Length; ++tailIndex)
            {
                var swap = _data[tailIndex];
                if (swap <= _data[nodeIndex]) continue;
                _data[tailIndex] = _data[nodeIndex];
                _data[nodeIndex] = swap;
                _rank = CalcRank(_data, _choices);
                return nodeIndex;
            }
            ...
            for (;;)
            {
                ...
                var tail = _data[nodeIndex + 1];
                for (var tailIndex = nodeIndex + 2; tailIndex < _data.Length; ++tailIndex)
                    if (_data[tailIndex] < _data[nodeIndex])
                        _data[tailIndex - 1] = _data[tailIndex];
                    else
                    {
                        _data[tailIndex - 1] = _data[nodeIndex];
                        _data[nodeIndex] = _data[tailIndex];
                        while (++tailIndex < _data.Length)
                            _data[tailIndex - 1] = _data[tailIndex];
                        _data[tailIndex - 1] = tail;
                        _rank = CalcRank(_data, _choices);
                        return nodeIndex;
                    }
                if (tail > _data[nodeIndex])
                {
                    _data[_data.Length - 1] = _data[nodeIndex];
                    _data[nodeIndex] = tail;
                    _rank = CalcRank(_data, _choices);
                    return nodeIndex;
                }
                _data[_data.Length - 1] = tail;
            }
        }
        ...
        public IEnumerable<Permutation> GetRows()
        {
            ...
            {
                var startRank = Rank;
                for (var current = (Permutation) MemberwiseClone();;)
                {
        ...
        {
            for (var w = 1; w <= Choices; ++w)
            {
                var current = (Permutation) MemberwiseClone();
                current._data = new int[w];
                for (var ei = 0; ei < current._data.Length; ++ei)
                    current._data[ei] = ei;
                current._choices = w;
                current._rowCount = CalcCount(w, w);
                current._rank = 0;
        ...
        {
            for (var w = 1; w <= Picks; ++w)
            {
                var current = (Permutation) MemberwiseClone();
                current._data = new int[w];
                for (var ei = 0; ei < current._data.Length; ++ei)
                    current._data[ei] = ei;
                current._rowCount = CalcCount(current.Choices, w);
                current._rank = 0;
        ...
        public IEnumerable<Permutation> GetRowsOfPlainChanges()
        {
            ...
            if (RowCount > 0)
            {
                ...
                var startRank = Rank;
                for (var plainRank = CalcPlainRank(_data);;)
                {
                    yield return current;

                    plainRank = (plainRank + 1)%RowCount;

                    CalcPlainUnrank(current._data, plainRank);
                    current._rank = CalcRank(_data, current.Choices);
        ...
        public static List<T> Permute<T>(Permutation arrangement, IList<T> source)
        {
            ...
            for (var ai = 0; ai < arrangement.Picks; ++ai)
