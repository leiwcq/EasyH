...
namespace EasyH.Core.Math
{
    ...
    /// 
    /// Use the <see cref="P:Common.Combination.Item(System.Int32)">indexer</see>
    ...
    /// 
    /// Use the <see cref="P:Permute">Permute</see> method to pick objects from a supplied array
    ...
    ///     <see cref="P:Common.Multicombination.Item(System.Int32)">indexer</see>.
    ///   </li>
    ///   <li>
    ...
    public class Combination :
        ...
        IEnumerable<int>
    {
        private int[] _data; // The picks for the current rank. Length is 'k'.
        private readonly int _choices; // Number of possible values 'n'.
        private long _rowCount; // Row count of the table of k-combinations.
        ...
        public Combination(int choices)
        {
            ...
            for (var ki = 0; ki < _data.Length; ++ki)
                _data[ki] = ki;

            _choices = choices;
            _rowCount = choices == 0 ? 0 : 1;
            _rank = 0;
        }
        ...
        public long Rank
        {
            ...
            set
            {
                ...
                if (value < 0)
                {
                    _rank = value%RowCount;
                    if (_rank < 0)
                        _rank += RowCount;
                }
                else
                    _rank = value < RowCount ? value : value%RowCount;

                //
                // Perform unranking:
                //

                var diminishingRank = RowCount - _rank - 1;
                var combinaticAtom = Choices;

                for (var ki = Picks; ki > 0; --ki)
                    for (;;)
                    {
                        --combinaticAtom;

                        var trialCount = Combinatoric.BinomialCoefficient(combinaticAtom, ki);
                        if (trialCount > diminishingRank) continue;
                        diminishingRank -= trialCount;
                        _data[Picks - ki] = Choices - combinaticAtom - 1;
                        break;
                    }
            }
        }
        ...
        public int CompareTo(Combination other)
        {
            ...
            result = Choices - other.Choices;
            if (result != 0) return result;
            var rankDiff = Rank - other.Rank;
        ...
        public IEnumerable<Combination> GetRows()
        {
            ...
            {
                var startRank = _rank;
                for (var current = (Combination) MemberwiseClone();;)
                {
